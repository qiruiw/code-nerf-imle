...
def build_ext(config, dist):
    ...
    if lapack_info:
        config.add_extension(name='_dci', sources=dci_sources, depends=dci_headers, include_dirs=['include'],
                             extra_info=lapack_info, extra_compile_args=['-fopenmp'], extra_link_args=['-lgomp'])
