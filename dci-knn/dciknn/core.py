...
import numpy as np
from ._dci import _dci_new, _dci_get_proj_vec, _dci_get_num_points, _dci_get_num_levels, _dci_add, _dci_query, \
    _dci_clear, _dci_reset
...
class DCI(object):
    ...
    @property
    def proj_vec(self):
        return ProtectedArray(self._proj_vec, when_writable=lambda _: self.num_points == 0,
                              write_error=lambda _: AttributeError(
                                  "can only set projection vectors when the database is empty"))
    ...
    def proj_vec(self, new_proj_vec):
        ...
        if new_proj_vec.shape != self._proj_vec.shape:
            raise ValueError(
                "mismatch between the expected shape of projection vectors (%s) and the supplied shape (%s)" % (
                repr(self._proj_vec.shape), repr(new_proj_vec.shape)))
    ...
    def _check_array(self, arr):
        if arr.shape[1] != self.dim:
            raise ValueError(
                "mismatch between array dimension (%d) and the declared dimension of this DCI instance (%d)" % (
    ...
    def _check_is_base_array(self, arr):
        ...
        if arr.base is not None:
            ...
            if arr_addr is None or arr_addr != base.data:
                raise ValueError(
                    "array must not be derived from another array, except via the transpose operator. Pass in the original array and specify the indices or make a copy of the derived array.")
    ...
    def _check_and_fix_indices(self, data, indices):
            ...
            if indices.ndim == 1:
                if indices.dtype == np.intc:
                    ...
                    if indices.shape[0] == data.shape[0]:
                        ...
                        raise IndexError(
                            "mismatch between the number of boolean indices (%d) and array dimension (%d)" % (
                            indices.shape[0], data.shape[0]))
                elif indices.dtype.kind in np.typecodes[
                    'AllInteger']:  # Check if dtype is an integer type; also returns true if dtype is bool
        ...
        if check_indices_within_bounds:
            if np.any(selected_idx < 0) or np.any(selected_idx >= data.shape[0]):
                raise IndexError("some indices (e.g. %d) out of bounds" % (
                indices[(selected_idx < 0) | (selected_idx >= data.shape[0])][0]))
    ...
    def add(self, data, indices=None, num_levels=2, field_of_view=10, blind=False, num_to_visit=-1, num_to_retrieve=-1,
            prop_to_visit=-1.0, prop_to_retrieve=-1.0):

        num_points = self.num_points

        if num_points > 0:
            raise RuntimeError(
                "DCI class does not support insertion of more than one array. Must combine all arrays into one array before inserting")
        ...
        if is_contiguous:
            _dci_add(self._dci_inst, data, _indices[0], _indices[1], num_levels, blind, num_to_visit, num_to_retrieve,
                     prop_to_visit, prop_to_retrieve, field_of_view)
        else:
            selected_data = data[_indices]
            _dci_add(self._dci_inst, selected_data, 0, _indices.shape[0], num_levels, blind, num_to_visit,
                     num_to_retrieve, prop_to_visit, prop_to_retrieve, field_of_view)
            self._orig_indices = _indices

        self._array = data

    # query is num_queries x dim
    def query(self, query, num_neighbours=-1, field_of_view=100, blind=False, num_to_visit=-1, num_to_retrieve=-1,
              prop_to_visit=-1.0, prop_to_retrieve=-1.0):
        ...
        _nearest_neighbour_idx, _nearest_neighbour_dists, num_candidates = _dci_query(self._dci_inst, _query,
                                                                                      num_neighbours, blind,
                                                                                      num_to_visit, num_to_retrieve,
                                                                                      prop_to_visit, prop_to_retrieve,
                                                                                      field_of_view)
